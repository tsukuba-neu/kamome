...
- `sendDailyReminders` - Posts daily event reminders to Discord channel via REST API (15:00 cutoff for today/tomorrow)

**Services** ([src/services/](src/services/)):
- `discord.ts` - Fetches messages via Cloud Run proxy (uses GAS identity token for auth), posts messages via Discord REST API
...
- `DISCORD_PROXY_URL`, `DISCORD_BOT_TOKEN`, `SOURCE_CHANNEL_ID`
- `NOTIFICATION_CHANNEL_ID`, `CALENDAR_ID`, `GEMINI_API_KEY`
